...
//   - enum：枚举值校验
//
// 生成文件：domain/service/{AggregateName}ServiceImpl.go
...
func (g *ServiceImplGenerator) Generate(agg *metadata.AggregateMetadata, outputDir string) error {
	// 创建输出目录
	serviceDir := filepath.Join(outputDir, "domain", "service")
...
func (g *ServiceImplGenerator) generateCode(agg *metadata.AggregateMetadata) string {
	...
	sb.WriteString(fmt.Sprintf("var _ %sService = (*%sServiceImpl)(nil)\n",
		agg.Name, agg.Name))
