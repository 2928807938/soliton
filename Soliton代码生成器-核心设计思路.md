...
```go
// 方式一：生成器自动生成（推荐）
// 用户只需定义聚合根，生成器自动生成 Entity 方法到独立文件
...
// ✅ 生成器自动生成：domain/model/Order_entity.go
...
| 标记 | 生成校验逻辑 | 位置 |
|------|-------------|------|
| `+soliton:required` | 非空校验 | Add/Update |
| `+soliton:unique` | 唯一性校验 | Add/Update（Update 时排除自己） |
| `+soliton:ref` | 外键存在性校验 | Add/Update |
| `+soliton:enum` | 枚举值校验 | Add/Update |
